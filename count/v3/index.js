...
$('#source').on('input', function (e) {
	...
	w = worker('split', function () {
		...
		onmessage = function ({ data }) {
			...
			postMessage({ rows, rowsLength, totalLength, innerLength , res })
			close()
		}
	}, function (data) {
		([ 'totalLength', 'innerLength', 'rowsLength', ]) .forEach(id=>{
			$('#'+id).text(data[id])
		});
	...
	timeout = setTimeout(()=>{
		...
		if(v) {
			...
			([ 'totalLength', 'innerLength', 'rowsLength', 'n', 'en', 'cn', 'kr', 'jp' ]).forEach(id=>{
				$(`#${id}`).text('0')
			});
		}
	})
